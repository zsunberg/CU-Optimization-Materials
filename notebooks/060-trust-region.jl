...
function dogleg_trust_region_search(f, x0=[0.0, 0.0];
							 δ=1.0, η1=0.25, η2=0.5, γ1=0.5, γ2=2.0, n=100, ϵg=1e-4)
	x = float(x0)
	history = []
	y = f(x)
	for _ in 1:n
		g = first(gradient(f, x))
		if norm(g) < ϵg
			break
		end
		H = hessian(f, x)

		if isposdef(H) # use dogleg
			xH = x - H\g
			if norm(xH - x) <= δ # Case 1 
				xp = xH
			else
				xC = x - dot(g, g)/dot(g, H*g)*g
				if norm(xC - x) >= δ # Case 2
					xp = x + (xC - x)*δ/norm(xC-x)
				else # Case 3
					a = norm(xH - xC)^2
					b = 2*sum((xC-x).*(xH-xC))
					c = norm(xC - x)^2 - δ^2
					tauH = (-b + sqrt(b^2-4*a*c))/(2*a)
					xp = xC + tauH*(xH-xC)
				end
			end
		else # just follow the gradient to the trust region boundary
			xp = x - δ * g / norm(g)
		end

		push!(history, (; x, y, g, H, δ, xp))

		# Adjust trust region
		yhat = y + dot(g, (xp-x)) + dot(xp-x, H*(xp-x))
		yp = f(xp)
		r = (y - yp) / (y - yhat)
		if yp >= y || r < η1
			δ *= γ1
...
history = dogleg_trust_region_search(f, x0)

# ╔═╡ 0218df75-241d-497d-af1c-e27bfc18bf42
begin
	plot()
	mycontour!(f)
	xhistory = getindex.(history, :x)
	plot!(first.(xhistory), last.(xhistory), xlim=(-2,2), ylim=(-2.5, 2.5), marker=true, label=nothing)
	plot!()
end
